...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:d="http://docbook.org/ns/docbook" xmlns:l="http://docbook.sourceforge.net/xmlns/l10n/1.0" version="1.0">
  ...
  <xsl:template name="chapter.titlepage.recto">
    <fo:block start-indent="0pt" background-color="{$primary.main.color}" border-width="3pt" border-color="{$primary.main.color}" border-style="solid">
      <fo:table inline-progression-dimension="100%">
        <fo:table-body>
          <fo:table-row>
            <fo:table-cell start-indent="0pt" width="{$body.start.indent}" padding-right="{$body.start.indent}" display-align="center">
              <fo:block>
                <fo:external-graphic content-height="{$logo.height}" src="url({$logo})"/>
              </fo:block>
            </fo:table-cell>
            <fo:table-cell>
  ...
  <xsl:template name="chapappendix.title">
    ...
    <fo:block background-color="{$primary.main.color}">
      <fo:block>
