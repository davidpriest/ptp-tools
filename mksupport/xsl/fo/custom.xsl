...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" version="1.0">
  ...
  </xsl:template>  <xsl:template match="comment[&comment.block.parents;]|remark[&comment.block.parents;]">
  ...
  <xsl:template match="simpara[child::indexterm[not(preceding-sibling::*) and not(following-sibling::*)]]">
    <!-- deal with awkward inclusion of non-inline "inline" index terms ... ie. on their own line, separated by blank lines above and below -->
    <xsl:apply-templates/>
  </xsl:template>
  ...
  <xsl:template match="phrase[@role='arrow.separator']">
    <fo:inline font-family="'Symbol,ZapfDingbats,UnicodeSymbols'"><xsl:apply-templates/></fo:inline>
  </xsl:template>
</xsl:stylesheet>
