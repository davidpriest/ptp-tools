		...
		<fo:simple-page-master page-height="{$page.height}" page-width="{$page.width}" master-name="fullpage">
			<xsl:choose>
				<xsl:when test="$draft.mode='yes'">
					<fo:region-body background-image="{$DOCTOOLS}/mksupport-common/images/cover-background-draft.svg" background-position-vertical="1.75in" background-repeat="no-repeat" margin="0in" overflow="visible"/>
				</xsl:when>
				<xsl:otherwise>
					<fo:region-body background-image="{$DOCTOOLS}/mksupport-common/images/cover-background-release.svg" background-position-vertical="1.75in" background-repeat="no-repeat" margin="0in" overflow="visible"/>
				</xsl:otherwise>
			</xsl:choose>
